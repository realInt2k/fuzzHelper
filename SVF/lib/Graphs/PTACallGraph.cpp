...
{
    //llvm::errs() << "int2k debug: In addCallGraphNode(): adding " << (*fun) << "\n";
    writeGraph("int2kGraph.txt") << (*fun) << "\n";
...
void PTACallGraph::addDirectCallGraphEdge(const CallBlockNode* cs,const SVFFunction* callerFun, const SVFFunction* calleeFun)
{
    ...
    //llvm::errs() << "int2k debug: In addDirectCallGraphEdge(): connecting " << 
    //    (*callerFun) << " -> " << (*calleeFun) << "\n";
    
    llvm::Function *int2kCaller = callerFun->getLLVMFun();
    llvm::Function *int2kCallee = calleeFun->getLLVMFun();

    llvm::Value *int2k = int2kCaller;
    /*llvm::errs() << " it's an instance of Value? " << (std::is_base_of<Value, Function>::value) << "\n";*/
    /*std::error_code EC;
    llvm::raw_ostream *out = new raw_fd_ostream("what.txt", EC, llvm::sys::fs::F_None);
    (*out) << "hello from the other siddeeee\n";
    (*out) << " " << (*callerFun) << "\n";
    delete out; */
    //int2k->print(*out, false);

    writeGraph("int2kGraph.txt") << (*callerFun) << " " << (*calleeFun) << "\n";
    //writeGraph("int2kGraph.txt") << (*callerFun) << " " << (*calleeFun) << "\n";
