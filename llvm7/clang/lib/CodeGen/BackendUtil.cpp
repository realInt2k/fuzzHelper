...
static void addHandler8Pass(const PassManagerBuilder &Builder,
                          PassManagerBase &PM) {
  PM.add(createHandler8Pass());
} 

static void addHandler8HelperPass(const PassManagerBuilder &Builder,
                                PassManagerBase &PM) {
  PM.add(createHandler8HelperPass());
}
...
void EmitAssemblyHelper::CreatePasses(legacy::PassManager &MPM,
  ...
  if (LangOpts.Sanitize.has(SanitizerKind::Handler8Helper)) {
    PMBuilder.addExtension(PassManagerBuilder::EP_ModuleOptimizerEarly,
                               addHandler8HelperPass);
    PMBuilder.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,
                               addHandler8HelperPass);
  }
    

  if (LangOpts.Sanitize.has(SanitizerKind::Handler8)) {
    PMBuilder.addExtension(PassManagerBuilder::EP_ModuleOptimizerEarly,
                               addHandler8Pass);
    PMBuilder.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,
                               addHandler8Pass);
