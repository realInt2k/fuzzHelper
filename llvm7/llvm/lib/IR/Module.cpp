...
#include <stdlib.h> // for int system(const char * command)
...
#include <string>
#include <cstdio>
// int2k defined functions
std::map<std::string, int> nodeID; // from name to ID
std::string *nodeName;
std::vector<int> *adj;
std::map<int, Function*> funcAddr; // from ID to address
bool *visited;
void dfs(int x)
{
  visited[x] = true;
...
void Module::injectSVFInfo()
{
  ...
  for(auto it = Module::FunctionList.begin(), eit = Module::FunctionList.end();
    it != eit; it ++)
    {
      Function *func = &*it;
  ...
  for(auto it = Module::FunctionList.begin(), eit = Module::FunctionList.end();
    it != eit; it ++)
    {
      ...
      dfs(id);
      for(int j = 0; j < Module::_nNode; ++j)
      {
        if(!visited[j])
          func->unrelatedFunc.push_back(funcAddr[j]);
      }
    }
    freemem();
}
...
void Module::readSVF()
{
  ...
  file >> Module::_nNode >> Module::_nEdge;
  int n = Module::_nNode, m = Module::_nEdge;
  file.close();
  ...
  for(int i = 0; i < n; ++i)
  {
    ...
    int ID = nodeID[x];
    nodeName[ID] = x;
    Module::needInstrumentedFunc.push_back(x);
  }
  for(int i = 0; i < m; ++i)
  {
    file >> x >> y;
...
Module::Module(StringRef MID, LLVMContext &C)
  ...
  //llvm::errs() << " here: " << (this) << " " << SourceFileName << "\n";
}

Module::~Module() {
  //llvm::errs() <<" dele : " << (this) << "\n";
  if(!this->postCmd.empty()) {
    system(this->postCmd.c_str());
  }
